...
@section('title')
	{{__('Tour Checkout')}} | {{$gs->website_title}}
@endsection
@section('content')
                ...
                <div class="row">
                                            ...
                                            <div class="row ">
                                                <div class="col-md-6">
                                                    <div class="form-group">
                                                        ...
                                                        <input type="text" class="form-control card-elements" name="cardNumber" placeholder="{{ __('Card Number')}}" autocomplete="off" id="validateCard">
                                                ...
                                                <div class="col-md-6">
                                                    <div class="form-group">
                                                        ...
                                                        <input type="text" class="form-control card-elements"  placeholder="{{__('Cvc')}}" name="cardCVC" id="validateCVC">
                    ...
                    <div class="col-lg-4">
                        <div class="booking-info">
                            <div class="top-area">
                                <h5 class="name">{{$tour['tour']['title']}}</h5>
                                <p class="location"><i class="fas fa-map-marker-alt"></i> {{$tour['tour']->country->country}}, {{$tour['tour']->state->state}}</p>
                                <div class="image">
                                    <img src="{{asset('assets/images/tour/feature-image/'.$tour['tour']->image->image)}}" alt="">
                                </div>
                            </div>
                            <div class="area-two">
                                <ul>
                            ...
                            <div class="area-three">
                                ...
                                <ul>
                                    @foreach ($tour['fac_name'] as $key => $e_p_name)
                                    <li>
                                        <span>{{$e_p_name}}: <small>({{str_replace('_',' ',$tour['fac_type'][$key])}})</small></span>
                                        <span>{{PriceHelper::showCurrency()}} {{PriceHelper::showPrice($tour['fac_price'][$key])}}</span>
                                    </li>
                                    @endforeach
                                </ul>
                            </div>
                            ...
                            <div class="area-three">
                                ...
                                <ul>
                                    @foreach ($tour['person_type'] as $key => $t_p_type)
                                    <li>
                                        <span>{{$t_p_type}} * {{$tour['person_qty'][$key]}} : </span>
                                        <span>{{PriceHelper::showCurrency()}}{{PriceHelper::showPrice($tour['person_price'][$key])}}</span>
                                    </li>
                                    @endforeach
                                </ul>
                            </div>
                            ...
                            <div class="area-four">
                                ...
                                <h4 class="total">
                                    {{PriceHelper::showCurrency()}} {{PriceHelper::showPrice($tour['total'])}}
                                </h4>
                            </div>
                        </div>
                    </div>
       ...
       <input type="hidden" id="tour_stripe_route" value="{{route('tour.stripe.submit')}}">
       <input type="hidden" id="tour_offline_route" value="{{route('tour.offline.payment')}}">
       <input type="hidden" id="tour_instamojo_route" value="{{route('tour.instamojo.payment')}}">
       <input type="hidden" id="tour_paypal_route" value="{{route('tour.paypal.payment')}}">
       <input type="hidden" id="tour_authorize_route" value="{{route('tour.authorize.payment')}}">
       <input type="hidden" id="tour_mollie_route" value="{{route('tour.mollie.payment')}}">
       <input type="hidden" id="tour_paystack_route" value="{{route('tour.paystack.payment')}}">
       <input type="hidden" id="tour_mercadopago_route" value="{{route('tour.mercadopago.payment')}}">
       <input type="hidden" id="tour_rezorpay_route" value="{{route('tour.rezorpay.payment')}}">
...
@endphp
<input type="hidden" value="{{$mercadopagoData['public_key']}}" id="tourmercadopagokey">
...
<script src="https://js.paystack.co/v1/inline.js"></script>
    ...
    <script src="{{asset('assets/front/js/tour/index.js')}}"></script>
  
    <script>
        
        $(document).on('submit','.step1-form',function(){
           
            var total = "{{round(PriceHelper::showPrice($tour['total']))}}";
