    ...
    jQuery(document).ready(function () {
		...
		$(document).on('change', '#sort_by', function () {
			$('.ajax_loader').show();
			 sort = $(this).val();
			$('#tour .sort').val(sort);
			$("#car_ajax_load").load(mainurl + '/cars?country_id=' + country + '&review=' + review + '&sort=' + sort + '&view=' + view + '&minPrice=' + minPrice + '&maxPrice=' + maxPrice + '&page_type=false');
		...
		$(document).on('click', '.pagination .page-item .page-link', function (e) {
			...
			if(paginatelink){
			$("#car_ajax_load").load($(this).attr('href')+'&country_id='+country+'&review='+review+'&sort='+sort+'&view='+view+'&minPrice='+minPrice+'&maxPrice='+maxPrice+'&page_type=false');
			}
	...
	// Checkout
		...
		function checkMercadopago(value) {
			if (value == 1) {
				...
				window.Mercadopago.setPublishableKey($('#carmercadopagokey').val());
